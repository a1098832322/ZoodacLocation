...
/**
 * 这是对“聚英®继电器板”驱动包封装的方法，打开和关闭继电器调用：writeDigitalOutput 方法
 * 
 * @author 郑龙
 *
 */
public class ModbusUtil {
	...
	public static void writeRegister(String ip, int port, int slaveId, int address, int value) {
		...
		try {
			 ...
			 * address, register);
			 * 
			 * req.setUnitID(slaveId); trans.setRequest(req);
			 * 
			 * System.out.println("ModbusSlave: FC" + req.getFunctionCode() +
			 * " ref=" + req.getReference() + " value=" + register.getValue());
			 */
			trans.execute();
	 ...
	 * @throws Exception
	 */
	public static void writeDigitalOutput(String ip, int port, int slaveId, int address, int value) throws Exception {
